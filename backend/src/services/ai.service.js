...
function mapGenAiError(error) {
    const message = normalizeErrorMessage(error);
    const status = Number(error?.status);
    const loweredMessage = message.toLowerCase();

    if (message.includes("GOOGLE_GENAI_API_KEY or GEMINI_API_KEY is not set")) {
        return {
            status: 500,
            code: "AI_API_KEY_MISSING",
            message: "AI service is not configured. Set GOOGLE_GENAI_API_KEY or GEMINI_API_KEY in backend environment variables."
        };
    }

    const leakedOrRevokedKey =
        status === 403 &&
        (loweredMessage.includes("reported as leaked") ||
            loweredMessage.includes("permission_denied") ||
            loweredMessage.includes("api key") ||
            loweredMessage.includes("invalid api key") ||
            loweredMessage.includes("api_key_invalid"));

    if (leakedOrRevokedKey) {
        return {
            status: 502,
            code: "AI_API_KEY_INVALID_OR_LEAKED",
            message: "Gemini API key is invalid, revoked, or reported as leaked. Create a new API key, update GOOGLE_GENAI_API_KEY (or GEMINI_API_KEY), and restart the backend server."
        };
    }
    ...
    return {
        ...
        message: "Failed to generate AI response from Gemini. Try again shortly."
    };
}
